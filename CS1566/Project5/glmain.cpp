...
const GLfloat coneColor[3] = {1.0, 0.0, 0.0};
const GLfloat cubeColor[3] = {1.0, 1.0, 0.0};
const GLfloat sphereColor[3] = {0.0, 1.0, 1.0};
const GLfloat cylinderColor[3] = {0.0, 0.0, 1.0};
const GLfloat torusColor[3] = {1.0, 0.0, 0.5};
...
void translateCone(GLdouble, GLdouble, GLdouble);
void rotateCone(GLdouble, GLdouble, GLdouble, GLdouble);
...
{ 
  ...
  //my_shapes.push_front(Shape(SHAPE_CONE, 0, 10, 20, 5)); //initial cone
  glutMainLoop();
  return(0);
}
...
void parse_obj(char *buffer)
{
  ...
  case 1: //cube
    ...
    my_shapes.push_front(Shape(SHAPE_CUBE, 0, 0, 0, 1.0, cubeColor));
  ...
  case 3: //sphere
    ...
    my_shapes.push_front(Shape(SHAPE_SPHERE, 0.0, crt_rs, crt_vs, 1.0, sphereColor));
    my_shapes[0].tessellate();
    break;

  case 4: //cylinder
    printf("Constructing Cylinder...\n");
    my_shapes.push_front(Shape(SHAPE_CYLINDER, 0.0, crt_rs, crt_vs, cyl_height, cylinderColor));
    my_shapes[0].tessellate();
    break;

  case 5: //cone
    printf("Constructing Cone...\n");
    my_shapes.push_front(Shape(SHAPE_CONE, 0.0, crt_rs, crt_vs, cyl_height, coneColor));
  my_shapes[0].tessellate();
    break;

  case 6: //torus
    printf("Constructing Torus...\n");
  my_shapes.push_front(Shape(SHAPE_TORUS, 0.0, crt_rs, crt_vs, 1.0, torusColor)); 
  my_shapes[0].tessellate();
  my_shapes[0].translate(0, -.5, 0);
  my_shapes[0].scale(1, 1, .8);
  my_shapes[0].scale(1, 1, .8);
  my_shapes[0].scale(.8, 1, 1);
  my_shapes[0].scale(.8, .9, .3);
  my_shapes[0].scale(1, .5, 1);
 
    break;
  }

  centerCone = Shape(SHAPE_CONE, 0.0, 30, 40, 2, coneColor);
  centerCone.tessellate();
  //centerCone.translate(0, -1, 0);
  //centerCone.translate(0, -1, 0);
  //centerCone.translate(0, -1, 0);
  printf("INSIDE OF parse_obj\n");

  //RT = RayTracer(&centerCone, GLOBAL_FAR);
  //printf("end RT setup\n");
...
void parse_camera(char *buffer)
{
  ...
  //myCamera.translateCamera(0.0, 18.0, 0.0);
  /*glLoadIdentity();
...
void my_keyboard( unsigned char key, int x, int y ) {
  ...
  case 'a': {
    switch (translation_type) {
      case 0: {
        rotateCameraLocalized(-5, 0, 1, 0); break;
      }; break;

      case 1: {
        myCamera.synchronizedRotate(1, &centerCone); break;
      }; break;
    } 
    glutPostRedisplay();
  }; break;
  ...
  case 'w': {
    ...
    {
      case 0: {
        translateCameraLocalized(0, 0, -1); 
        centerCone.translate(0, 0, 1);
      }; break;

      case 1: {
        myCamera.synchronizedTranslate(0, 0, 1, &centerCone); break;
      }; break;
    }
    glutPostRedisplay();
  }; break;
  ...
  case 's': {
    ...
    //rotateCameraLocalized(1, 0, 1, 0);
    //myCamera.synchronizedRotate(-1, &centerCone);
    glutPostRedisplay();
  }; break;
  ...
  case 'f': {
    ...
    GLdouble *temp = myCamera.getPosition();
    bool *hits = centerCone.fireRay(&my_shapes, &indicatorList);
    for (int i = 0; i < my_shapes.size(); i++)
    {
      if (hits[i]==true)
      {
        if (my_shapes[i].getType()==SHAPE_SPHERE)
        {
          //printf("SPHERE HIT!\n");
        }
        else
        {
          //printf("OTHER SHAPE HIT!\n");
        }
      }
    }
    delete[] hits;
  ...
  //W NEEDED FOR MOVEMENT CONTROLS (AWSD)
  /*case 'w': {
  switch( crt_transform) {
...
void special(int key, int x, int y) //this function handles GLUT arrow key controls
{
  switch(key)
  {
    case GLUT_KEY_UP: {
    ...
    case GLUT_KEY_RIGHT: {
      ...
      {
        case 0: {
          centerCone.translate(1, 0, 0);
        }; break;

        case CONE_ROTATION_MODE: {
          centerCone.revolve(.75, 0, 1, 0);
        }; break;
      }
      glutPostRedisplay();
    }; break;
...
void my_raytrace(int mousex, int mousey)
{
  ...
  //glMatrixMode(GL_MODELVIEW);
  //RT.fire(&my_shapes, mousex, mousey);
  //glutPostRedisplay();
  //glutSwapBuffers();

  printf("RT stuff complete\n");
...
void draw_objects() 
{
  GLfloat indicatorColor[3] = {1.0, 0.0, 0.0};
  //crt_render_mode = GL_LINE_LOOP;
  int i;
  for(i=0; i<my_shapes.size(); i++)
  {
    if (normalFlag==1)my_shapes[i].draw(crt_render_mode, true, colors[BLUE]); //for each shape in deque, draw it
    else my_shapes[i].draw(crt_render_mode, false, colors[BLUE]);
  }
  centerCone.draw(crt_render_mode, true, coneColor);

  if (indicatorList.empty()!=true)
  {
    for (std::deque<Shape>::iterator iter = indicatorList.begin(); iter != indicatorList.end(); iter++)
    {
      iter->draw(crt_render_mode, false, indicatorColor);
    }
  }
}
...
void translateCone(GLdouble x, GLdouble y, GLdouble z)
{
  printf("translating cone by: %g, %g, %g \n", x, y, z);
  centerCone.translate(x, y, z);
  centerCone.tessellate();
  //glutPostRedisplay();
}

void rotateCone(GLdouble deg, GLdouble x, GLdouble y, GLdouble z)
{
...
void my_display() {
  ...
  if (coneFlag==0)
  {
    centerCone.translate(-5.0, 0.0, 0.0);
    //const GLfloat coneColor[3] = {1.0, 0.0, 0.0};
    /*centerCone = Shape(SHAPE_CONE, 0.0, 30, 40, 2, coneColor);
    centerCone.tessellate();
    centerCone.draw(crt_render_mode, true, coneColor);
    
    //myCamera.linkObject(centerCone, -5.0, 0.0, -5.0, crt_render_mode, normalFlag);


    my_shapes.push_front(centerCone);
    my_shapes[0].tessellate();
  
    if (normalFlag==1) my_shapes[0].draw(crt_render_mode, true, coneColor);
    else my_shapes[0].draw(crt_render_mode, false, coneColor);

    my_shapes[0].translate(-5.0, 0.0, -5.0);
    */
    coneFlag=1;
  }
...
void my_TimeOut(int id) 
{
  ...
  if (initFlag!=0)
  {
    myCamera.translateCamera(0.0, dy, 0.0);
    //centerCone.translate(0.0, dy, 0.0);
    dy+=1.0;
    if (dy>6.0)
    {
      initFlag=0;
  ...
  if (jumpFlag != 0)
  {
    ...
    //centerCone.jump();
    //glutPostRedisplay();    
    //if (myCamera.dy>0.5) jumpFlag=0;
  }

  //centerCone.revolve(.25, 0, 1, 0);
 for (std::deque<Shape>::iterator it = my_shapes.begin(); it < my_shapes.end(); it++)
  {
    if (it->getType()==SHAPE_TORUS)
    {
      it->revolve(3, 0, 1, 0);
    }
  }
  ...
  glutTimerFunc(30, my_TimeOut, 0);/* schedule next timer event, 20 secs from now */
